...
func New(kubeConfigPath string) (*Framework, CleanUpFunc, error) {
	...
	operatorClient, err := client.NewForConfig(config, "", namespaceName, userWorkloadNamespaceName, client.EventRecorder(events.NewInMemoryRecorder("cluster-monitoring-operator", clocktesting.NewFakePassiveClock(time.Now()))))
	if err != nil {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("creating apiextensions client: %w", err)
	}

	vpaClient, err := vpaclient.NewForConfig(config)
	if err != nil {
		return nil, nil, fmt.Errorf("creating vpa client: %w", err)
	}
